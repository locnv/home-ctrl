...
(function() {
  ...
  function ServiceImpl( logger ) {
    ...
    function addSpeechRecognized(listener) {
      let idx = mOnSpeechRecognized.indexOf(listener);
      if(idx !== -1) {
    ...
    function notifySpeechRecognized(rs) {
      for(let i = 0; i < mOnSpeechRecognized.length; i++) {
        let fn = mOnSpeechRecognized[i];
        fn.call(null, rs);
      }
    }
    ...
    function onResult(event) {
      ...
      /*let nbRs = event.results.length;
      for(let i = 0; i < nbRs; i++) {
        let rs = event.results[i][0];
        let speechResult = rs.transcript.toLowerCase();

        logger.debug('Result: ' + speechResult +' (Confidence: ' + rs.confidence+')');
      }*/

      let nbRs = event.results.length;
      let rs = event.results[nbRs-1][0];
      let speechResult = rs.transcript.toLowerCase();
