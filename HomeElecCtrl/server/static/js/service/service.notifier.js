...
(function() {
  ...
  function ServiceImpl(LogService, Util) {
    ...
    /* Local variables */
    let mIsClosed = true;
    let mlogger = LogService;
    let mUtil = Util;

    // Native notifier
    let mSysNotifier = null;

    // Local notification
    let mLocalNotifier = null;
    ...
    function checkSysNotifier() {
      ...
      if(mSysNotifier === null && mUtil.isBrowser()) {
        mSysNotifier = new BrowserNotifier();
      }
    ...
    function BrowserNotifier() {
      ...
      function confirm(message, fCallback, title, btnLabels) {
        ...
        fCallback.call(null, ret);
      }

      function prompt(message, fCallback, title, btnLabels, defText) {
        let ret = window.prompt(message);
        fCallback.call(null, ret);
